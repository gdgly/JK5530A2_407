...
void Transformation_ADC(void)  
{
	...
	vu32 var32a;
/*****************************负载测量电压转换******************************************/
	if(CLists.CLOAD_V_SW==0)//低档
	{
		var32 = Vmon_Load_value;
		var32 = var32 * CLists.CREG_Load_CorrectionV_LOW;  
		...
		CLists.CVoltage = var32;
		DISS_Voltage=CLists.CVoltage;
...
// 	//如果DISS_Voltage的值  >=10 CLists.CLOAD_V_SW  =1
	...
	{
		var32 = Vmon_Load_value;
		var32 = var32 * CLists.CREG_Load_CorrectionV_HIG;  
...
// 		}
   //如果DISS_Voltage的值  <12  CLists.CLOAD_V_SW  = 0
	...
	/*******************负载测量电流转换**************************************/
	if(CLists.CLOAD_I_SW==0)//低档
	{
		var32 = Imon_Load_value;
		var32 = var32 * CLists.CREG_Load_CorrectionA_LOW;  
		...
		var32 = var32 >> 12;
		CLists.CLaod_Current = var32;
		DISS_Current=CLists.CLaod_Current;
	...
	/*************************负载电压和电流控制转换**************************************/
	if(CLists.CLOAD_MODE==0)//CC
	{
		if(CLists.CLOAD_I_SW==0)//低档
		{
			var32 = CLists.CSET_Current_Laod;
			var32=var32<<12;   
	...
	else if(CLists.CLOAD_MODE==1)//CV
	{
		if(CLists.CLOAD_V_SW==0)//低档
		{
			var32 = CLists.CSET_Voltage_Laod;
			var32=var32<<12;   
			if ((CLists.CPolar2 & 0x04) == 0)			   
			{
...
/*****************************内阻值转换*******************************************/
	if(CLists.CRCONT_SW==0)//低档
	{
		var32 = Rmon_value;
		var32 = var32 * CLists.CREG_CorrectionR_LOW;  
		if ((CLists.CPolar4 & 0x01) == 0x01)		  
		{
		...
		CLists.CR_VLUE = var32;
		var32 = 0;	
	}
	else//高档
	{
		var32 = Rmon_value;
		var32 = var32 * CLists.CREG_CorrectionR_HIG;  
	...
	var32 = var32 * CLists.CREG_POWER_CorrectionV;  
	...
	CLists.CPOW_Voltage = var32;
	DISS_POW_Voltage=CLists.CPOW_Voltage;
	DISS_POW_Voltage=DISS_POW_Voltage/1000;//计算显示电压
	var32 = 0;
/*****************************稳压电源测量电流转换*******************************************/
	var32 = Imon_POWE_value;
	if(flag_CC_MODE==1)
	{
		var32 = var32 * CLists.CREG_POWER_CorrectionA_CC;	   
		...
		var32 = var32 >> 12;
		CLists.CCurrent = var32;;
		DISS_POW_Current=CLists.CCurrent;
...
/**************************稳压电源设置电压转换******************************************/
	...
	var32=var32>>1;
	Contr_Voltage = var32;
	if(CLists.CSET_Voltage==0)
	{
		Contr_Voltage=0;
	}
	var32 = 0;
/**************************稳压电源设置电流转换**************************************/
	var32 = CLists.CSET_Current;
	var32=var32<<12;   
...
/********************识别电阻1转换******************************/
	...
	//var32 = R1mon_value;
	var32 = var32 * CLists.CREG_CorrectionR1;  
	...
	CLists.CR1_VLUE = var32;
	var32 = 0;	
	
	
/********************识别电阻2转换******************************/	
	var32 = R2mon_value;
	var32 = var32 * CLists.CREG_CorrectionR2;  
